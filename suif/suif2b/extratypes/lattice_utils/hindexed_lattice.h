...
public:
  ...
  class iterator {
    ...
    suif_hash_map<IndexObj,SubLattice>::iterator _iter;
    suif_hash_map<IndexObj,SubLattice>::iterator _iter_end;
  public:
...
get_value(IndexObj v) const {
  suif_hash_map<IndexObj, SubLattice>::const_iterator iter =
    _values->find(v);
  if (iter == _values->end())
...
do_meet_with_test(const HIndexedLattice &other) {
  ...
  for (suif_hash_map<IndexObj, bool>::iterator it =
	 v.begin(); it != v.end(); it++) {
