...
public:
    ...
    tos_handle<elem_t> lookup_handle(elem_t the_elem) const
      {
        size_t the_count = count();
        for (size_t elem_num = 0; elem_num < the_count; ++elem_num)
          {
            if (elem_by_num(elem_num) == the_elem)
    ...
    elem_t head(void) const  { return elem_by_num((size_t)0); }
    elem_t tail(void) const
      {
        if (count() == 0)
            return zero((elem_t *)0);
        else
            return elem_by_num(count() - 1);
      }
